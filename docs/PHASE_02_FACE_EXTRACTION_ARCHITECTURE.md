...
- **RetinaFace**: State-of-the-art single-stage face detector. Extremely robust to occlusion and scale variance. Highly recommended for clinical/research fidelity.
- **MediaPipe Face Detection**: Highly optimized, incredibly fast, and very robust across varying demographics and poses. Strongly considered for this pipeline.

**Proposed Choice:** MediaPipe or RetinaFace.
...
**Proposed Choice:** Five-point alignment (if computationally feasible) or Eye-based alignment.
...
- **Face Too Small**: Discard frame if bounding box area is below a minimum threshold (e.g., less than 64x64).
